    ...
    for target in targets:
        for ratio in ratios:
            ...
            for ego_user in G_EGO_USERS:
                ...
                METHOD='gcn'
                savepath = res_dir0+ METHOD + '-disconnected_edges_sampled-'+target + Flag + '-' + str(ego_user)+'-new-629-lap-multi6.npy'
                print('***')
                disconnected_edges_sampled=np.load(savepath)


                METHOD = 'gcn'
                savepath = res_dir0 + METHOD + '-pos_sampled-'+target + Flag + '-' + str(ego_user) + '-new-629-lap-multi6.npy'
                print('***')
                ...
                for b in sigmas:
                    ...
                    # sigma = 50
                    if dp == 7:
                        Flag = '114-gat-' + str(ego_user) + '-' + str(dp) + '-' + str(b) + '-' + str(ratio)
                    ...
                    y_label_pos = np.concatenate(
                        (mem_class.reshape(-1, 1), np.ones((np.shape(triad_cliques)[0], 2)), triad_cliques),
                        axis=1)
                    non_mem = []
                    y_label_neg = []
                    ...
                    with open("{}/mlp_{}-b-new-629-imp-{}-{}-{}-multi6.pkl".format(res_dir,target,sigma,me,ratio), "wb") as f2:
                        pkl.dump(mlp, f2)
                    ...
                    result.to_csv("{}/mlp_sim-{}-b-new-629-imp-{}-{}-{}-multi6.csv".format(res_dir,target,sigma,me,ratio))
                ...
                result.to_csv("{}/pmia-results-{}-new-629-imp-{}-{}-multi-b6.csv".format(res_dir,target,me,ratio))
        ...
        for ratio in ratios:
            ...
            for ego_user in G_EGO_USERS:
                ...
                for b in sigmas:
                    ...
                    with open("{}/mlp_{}-w-new-629-imp-{}-{}-{}-multi6.pkl".format(res_dir,target,sigma,me,ratio), "wb") as f2:
                        pkl.dump(mlp, f2)
                    ...
                    name = ['y_score', 'prob', 'y_test_grd', 'y_test_class', '0']
                    result = pd.DataFrame(columns=name, data=tsts)
                    result.to_csv("{}/mlp_sim-{}-w-new-629-imp-{}-{}-{}-multi6.csv".format(res_dir,target,sigma,me,ratio))
                ...
                name = ['ego_user','b','ratio', 'acc', 'recall', 'precision', 'f1', 'auc']
                result = pd.DataFrame(columns=name, data=results_all)
                result.to_csv("{}/pmia-results-{}-new-629-imp-{}-{}-multi-w6.csv".format(res_dir,target,me,ratio))
