...
# from keras.models import Model Stevens755036
def sigmoid(x):
...
def add_laplace_noise(data_list, b=2, u=0):
...
for m_ in m_seed:
    ...
    for ego_user in G_EGO_USERS:
        ...
        res_dir = '%s/'%(ego_user)
        res_dir0='/pygcn-popets-triangle/%s/'%(ego_user)


        feat_dir = '/deepwalk-master/data/' + str(ego_user) + '-adj-feat.pkl'
        ...
        METHOD='gcn'
        savepath = res_dir0+ METHOD + '-disconnected_edges_sampled-4clique' + Flag + '-' + str(ego_user)+ '-new-629-lap-multi'+str(m_)+'.npy'
        print('***')
        disconnected_edges_sampled=np.load(savepath)

        METHOD = 'gcn'
        savepath = res_dir0 + METHOD + '-pos_sampled-4clique' + Flag + '-' + str(ego_user) + '-new-629-lap-multi'+str(m_)+'.npy'
        print('***')
        ...
        METHOD = 'gat'
        savepath = res_dir + METHOD + '-embeds1-' + Flag + '-' + str(ego_user) + '-layer2.npy'
        print('***')
        emb_matrix1 = np.load(savepath)

        savepath = res_dir + METHOD + '-embeds2-' + Flag + '-' + str(ego_user) + '-layer2.npy'
        print('***')
        emb_matrix2 = np.load(savepath)
        ...
        for path in cliqs:
            ...
            mem.append(prob)
            mem_class = np.array([2] * int(num_triad))
        ...
        #     [0] * int(idx/9) + [1] * int(idx/9) + [2] *int(idx/9) + [3] * int(idx/9) + [4] * int(idx/9) + [5] * idx + [6] * int(idx/9) + [7] * int(idx/9) + [8] * int(idx/9) + [9] * int(idx/9))

        non_mem_class = np.array([0] * int(5 * int(idx_min_ / 9)) + [1] * idx_min_ + [0] * int(4 * int(idx_min_ / 9)))
        ...
        y_label_pos = np.concatenate(
            (mem_class.reshape(-1, 1), np.ones((len(mem_class), 2)), np.array(cliqs)[mem_idx_sample]), axis=1)

        # y_label_pos=np.ones((np.shape(pos_sampled)[0],3))
        # y_label_neg=np.zeros((np.shape(neg_sampled)[0],3))
        neg_sampled0 = neg_sampled[0:int(num_triad / 9)]
        neg_sampled1 = neg_sampled[int(num_triad / 9):int(2 * num_triad / 9)]
        neg_sampled2 = neg_sampled[int(2 * num_triad / 9):int(3 * num_triad / 9)]
        neg_sampled3 = neg_sampled[int(3 * num_triad / 9):int(4 * num_triad / 9)]
        neg_sampled4 = neg_sampled[int(4 * num_triad / 9):int(5 * num_triad / 9)]
        neg_sampled5 = neg_sampled[int(5 * num_triad / 9):int(14 * num_triad / 9)]
        neg_sampled6 = neg_sampled[int(14 * num_triad / 9):int(15 * num_triad / 9)]
        neg_sampled7 = neg_sampled[int(15 * num_triad / 9):int(16 * num_triad / 9)]
        neg_sampled8 = neg_sampled[int(16 * num_triad / 9):int(17 * num_triad / 9)]
        neg_sampled9 = neg_sampled[int(17 * num_triad / 9):int(18 * num_triad / 9)]

        y_label_neg0 = y_label_neg[0:int(num_triad / 9)]
        y_label_neg1 = y_label_neg[int(num_triad / 9):int(2 * num_triad / 9)]
        y_label_neg2 = y_label_neg[int(2 * num_triad / 9):int(3 * num_triad / 9)]
        y_label_neg3 = y_label_neg[int(3 * num_triad / 9):int(4 * num_triad / 9)]
        y_label_neg4 = y_label_neg[int(4 * num_triad / 9):int(5 * num_triad / 9)]
        y_label_neg5 = y_label_neg[int(5 * num_triad / 9):int(14 * num_triad / 9)]
        y_label_neg6 = y_label_neg[int(14 * num_triad / 9):int(15 * num_triad / 9)]
        y_label_neg7 = y_label_neg[int(15 * num_triad / 9):int(16 * num_triad / 9)]
        y_label_neg8 = y_label_neg[int(16 * num_triad / 9):int(17 * num_triad / 9)]
        y_label_neg9 = y_label_neg[int(17 * num_triad / 9):int(18 * num_triad / 9)]
        ...
        X_train_train, X_train_test, y_train_train, y_train_test = train_test_split(pos_sampled, y_label_pos,
                                                                                    test_size=0.3, random_state=42)

        X_test_train0, X_test_test0, y_test_train0, y_test_test0 = train_test_split(neg_sampled0, y_label_neg0,
                                                                                    test_size=0.3, random_state=42)
        X_test_train1, X_test_test1, y_test_train1, y_test_test1 = train_test_split(neg_sampled1, y_label_neg1,
                                                                                    test_size=0.3, random_state=42)
        X_test_train2, X_test_test2, y_test_train2, y_test_test2 = train_test_split(neg_sampled2, y_label_neg2,
                                                                                    test_size=0.3, random_state=42)
        X_test_train3, X_test_test3, y_test_train3, y_test_test3 = train_test_split(neg_sampled3, y_label_neg3,
                                                                                    test_size=0.3, random_state=42)
        X_test_train4, X_test_test4, y_test_train4, y_test_test4 = train_test_split(neg_sampled4, y_label_neg4,
                                                                                    test_size=0.3, random_state=42)
        X_test_train5, X_test_test5, y_test_train5, y_test_test5 = train_test_split(neg_sampled5, y_label_neg5,
                                                                                    test_size=0.3, random_state=42)
        X_test_train6, X_test_test6, y_test_train6, y_test_test6 = train_test_split(neg_sampled6, y_label_neg6,
                                                                                    test_size=0.3, random_state=42)
        X_test_train7, X_test_test7, y_test_train7, y_test_test7 = train_test_split(neg_sampled7, y_label_neg7,
                                                                                    test_size=0.3, random_state=42)
        X_test_train8, X_test_test8, y_test_train8, y_test_test8 = train_test_split(neg_sampled8, y_label_neg8,
                                                                                    test_size=0.3, random_state=42)
        X_test_train9, X_test_test9, y_test_train9, y_test_test9 = train_test_split(neg_sampled9, y_label_neg9,
                                                                                    test_size=0.3, random_state=42)
        ...
        X_test = np.concatenate((X_train_test, X_test_test0, X_test_test1, X_test_test2, X_test_test3, X_test_test4,
                                 X_test_test5, X_test_test6, X_test_test7, X_test_test8, X_test_test9), axis=0)
        ...
        y_test = np.concatenate((y_train_test, y_test_test0, y_test_test1, y_test_test2, y_test_test3, y_test_test4,
                                 y_test_test5, y_test_test6, y_test_test7, y_test_test8, y_test_test9), axis=0)
        ...
        with open("{}/mlp_4cliq-w-new-629-multi-{}.pkl".format(res_dir, m_), "wb") as f2:
            pkl.dump(mlp, f2)
        ...
        for i in range(len(y_score)):
            ...
            tst = [y_score[i], proba[i], y_test[i][2], y_test[i][0], y_test[i][1]]
            tsts.append(tst)
        name = ['y_score', 'prob', 'y_test_grd', 'y_test_class', '0']
        result = pd.DataFrame(columns=name, data=tsts)
        result.to_csv("{}/mlp_sim-4clique-w-new-629-multi-{}.csv".format(res_dir, m_))

        y_0 = np.where(y_test[:, 0] == 0)[0]
        y_1 = np.where(y_test[:, 0] == 1)[0]
        y_2 = np.where(y_test[:, 0] == 2)[0]
        # y_3 = np.where(y_test[:, 0] == 3)[0]
        # y_4 = np.where(y_test[:, 0] == 4)[0]
        # y_5 = np.where(y_test[:, 0] == 5)[0]
        #
        # y_6 = np.where(y_test[:, 0] == 6)[0]
        # y_7 = np.where(y_test[:, 0] == 7)[0]
        # y_8 = np.where(y_test[:, 0] == 8)[0]
        # y_9 = np.where(y_test[:, 0] == 9)[0]
        # # y_10 = np.where(y_test[:, 0] == 10)[0]
        # y_12 = np.where(y_test[:, 0] == 12)[0]

        acc = accuracy_score(y_label_test[:, 0][y_0], y_score[y_0])
        ...
        acc = accuracy_score(y_label_test[:, 0][y_1], y_score[y_1])
        ...
        acc = accuracy_score(y_label_test[:, 0][y_2], y_score[y_2])
        ...
        # acc = accuracy_score(y_label_test[:, 0][y_3], y_score[y_3])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_4], y_score[y_4])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_5], y_score[y_5])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_6], y_score[y_6])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_7], y_score[y_7])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_8], y_score[y_8])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_9], y_score[y_9])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # # acc = accuracy_score(y_label_test[:, 2][y_10], y_score[y_10])
        # # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        # # results_att.append([args.dataset, acc, recall, precision, f1, auc])
        #
        # acc = accuracy_score(y_label_test[:, 0][y_12], y_score[y_12])
        # # recall = recall_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # precision = precision_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # f1 = f1_score(y_score[y_2], y_label_test[:, 2][y_2])
        # # auc = roc_auc_score(y_label_test[:, 2][y_2], proba[y_2])
        ...
        mlp.fit(X_train, y_train[:, 0])
        with open("{}/mlp_4cliq-b-new-629-multi-{}.pkl".format(res_dir, m_), "wb") as f2:
            pkl.dump(mlp, f2)
        ...
        name = ['y_score', 'prob', 'y_test_grd', 'y_test_class', '0']
        result = pd.DataFrame(columns=name, data=tsts)
        result.to_csv("{}/mlp_sim-4clique-b-new-629-multi-{}.csv".format(res_dir, m_))
    ...
    name = ['ego_user', 'acc', 'recall', 'precision', 'f1', 'auc']
    result = pd.DataFrame(columns=name, data=results_att)
    # result.to_csv("pmia-results-4cliq-new-629-multi.pkl")
    result.to_csv("{}/pmia-results-4cliq-new-629-multi-{}.csv".format(args.dataset, m_))
