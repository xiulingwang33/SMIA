...
for m_ in m_seed:
    ...
    for ego_user in G_EGO_USERS:
        ...
        METHOD='gcn'
        savepath = res_dir+ METHOD + '-disconnected_edges_sampled-4clique' + Flag + '-' + str(ego_user)+ '-new-629.npy'
        print('***')
        disconnected_edges_sampled=np.load(savepath)

        idx=int(np.shape(disconnected_edges_sampled)[0]/11)
        disconnected_edges_sampled=disconnected_edges_sampled[0:10*idx]

        METHOD = 'gcn'
        savepath = res_dir + METHOD + '-pos_sampled-4clique' + Flag + '-' + str(ego_user) + '-new-629.npy'
        print('***')
        cliqs=np.load(savepath)

        cliqs =cliqs[0:int(10*idx)]

        idx_min = int(np.shape(disconnected_edges_sampled)[0] / 10)
        if idx_min > 3000:
            idx_min_ = 3000
        else:
            idx_min_ = idx_min
        ...
        for ii in range(10):
            dis_new_ = disconnected_edges_sampled[int(ii * idx_min):int((ii + 1) * idx_min)]
            sample_new_ = np.array(random.sample(list(range(idx_min)), idx_min_))
            dis_new_ = dis_new_[sample_new_]

            dis_new.append(dis_new_)

        dis_new = np.array(list(itertools.chain.from_iterable(dis_new)))

        sample_new_ = np.array(random.sample(list(range(np.shape(cliqs)[0])), idx_min_))
        cliqs = cliqs[sample_new_]
        disconnected_edges_sampled = dis_new
        ...
        METHOD = 'gcn'
        savepath = res_dir + METHOD + '-disconnected_edges_sampled-4clique' + Flag + '-' + str(
            ego_user) + '-new-629-lap-multi'+str(m_)
        print('***')
        np.save(savepath, disconnected_edges_sampled)

        METHOD = 'gcn'
        savepath = res_dir + METHOD + '-pos_sampled-4clique' + Flag + '-' + str(ego_user) + '-new-629-lap-multi'+str(m_)
        print('***')
        ...
        non_mem_class = np.array(
            [0] * idx + [1] * idx + [2] * idx + [3] * idx + [4] * idx + [5] * idx + [6] * idx + [7] * idx + [8] * idx + [9] * idx)

        print('@@@', len(non_mem_class), np.shape(disconnected_edges_sampled)[0])

        y_label_neg = np.concatenate((non_mem_class.reshape(-1, 1), np.zeros((np.shape(disconnected_edges_sampled)[0], 2)),
        ...
        X_train = np.concatenate((X_train_train, X_test_train0, X_test_train1, X_test_train2, X_test_train3, X_test_train4, X_test_train5, X_test_train6, X_test_train7, X_test_train8, X_test_train9), axis=0)
        X_test = np.concatenate((X_train_test, X_test_test0, X_test_test1, X_test_test2, X_test_test3, X_test_test4, X_test_test5, X_test_test6, X_test_test7, X_test_test8, X_test_test9), axis=0)
        y_train = np.concatenate((y_train_train, y_test_train0, y_test_train1, y_test_train2, y_test_train3, y_test_train4, y_test_train5, y_test_train6, y_test_train7, y_test_train8, y_test_train9), axis=0)
        y_test = np.concatenate((y_train_test, y_test_test0, y_test_test1, y_test_test2, y_test_test3, y_test_test4, y_test_test5, y_test_test6, y_test_test7, y_test_test8, y_test_test9), axis=0)
        ...
        # non_mem_class=np.array([0]*int(num_triad/6)+[1]*int(num_triad/6)+[21]*int(num_triad/12)+[22]*int(num_triad/12)+[31]*int(num_triad/18)+[32]*int(num_triad/18)+[33]*int(num_triad/18)+[41]*int(num_triad/12)+[42]*int(num_triad/12)+[51]*int(num_triad/12)+[52]*int(num_triad/12))
        # non_mem_class = np.array(
        #     [0] * len(idx0) + [1] * len(idx1) + [2] * len(idx21) + [3] * len(idx22) + [4] * len(idx31) + [5] * len(
        #         idx32) + [6] * len(idx33) + [7] * len(idx41) + [8] * len(idx42) + [9] * len(idx51) + [10] * len(idx52))
